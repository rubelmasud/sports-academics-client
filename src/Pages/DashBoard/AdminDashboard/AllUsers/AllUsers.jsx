...
const AllUsers = () => {
    ...
    const { data: users = [], refetch } = useQuery(['users'], async () => {
        const res = await fetch('https://y-rubelmasud.vercel.app/users/')
        return res.json()
    })

    const handleMakeAdmin = user => {
        fetch(`https://y-rubelmasud.vercel.app/users/admin/${user._id}`, {
            method: "PATCH"
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handleMakeInstructor = user => {
        fetch(`https://y-rubelmasud.vercel.app/users/instructor/${user._id}`, {
            method: "PATCH"
        })
            .then(res => res.json())
            .then(data => {
